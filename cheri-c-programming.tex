...
While this model follows a single consistent rule, it can lead to surprising behaviour if an expression places the provenance-carrying value to the right-hand-side.
...
The current behaviour for such ambiguous cases is to select the left-hand-side as the provenance source, but we are considering making this an error in the future.
%
...
be able to optimise it to the equivalent \vaddrt form, provided the result
...
\begin{description}
\item[\ccode{size\_t \_\_builtin\_cheri\_round\_representable\_length(size\_t len)}] returns the length that a capability would have after using \ccode{cheri\_csetbounds} to set the length to \ccode{len} (assuming appropriate alignment of the base).
