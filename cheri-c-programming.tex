...
\lstnewenvironment{clisting}[1][]{\noindent\minipage{\linewidth}\lstset{language={C},breaklines=true,frame=L,#1}}{\endminipage}
\lstnewenvironment{compilerwarning}[1][]{\noindent\minipage{\linewidth}\lstset{language={},breaklines=true,basicstyle=\scriptsize\ttfamily\bfseries,frame=L,#1}}{\endminipage}
...
\item[Temporal Safety] prevents a pointer retained after the release of its
  ...
  \textit{revocation sweeps} to accurately and efficiently location and
...
stack.
Fail stop may be apropriate behavior in some environments, and is in fact the
...
Futher, any operation on the pointer passed to realloc is undefined upon
return.
...
Pointer value replacement may also be non-atomic, permitting some instances of
a pointer to continue to be usable for longer than others.
This model does permit limited \textit{use-after-free}, but prohibits
\textit{use-after-reallocation}.
...
\item[Unprototyped (\textit{K\&R}) functions] Because pointers can no longer
  ...
  makes function declerations with an empty parameter list equivalent to
...
\item[\cconst{CHERI\_PERM\_CHERIABI\_VMMAP}] A CheriABI-specific user
  permission which the kernel uses to authorize modifications to
