...
\Cref{sec:further_reading} recommends further reading, including our
...
and atomicity reasons, code depending on unaligned pointers will need
to be changed.
...
The use of these macros is now optional,\footnote{%
...
\end{lstlisting}
If you are allocating from a contiguous buffer, you will need to align the base upwards instead of down.
...
Usually, this warning is caused programmers incorrectly assuming that \clongt is able to store pointers.
...
The suggested fix for this problem is to insert a cast to an integer type:
\begin{clisting}[frame=single,numbers=left]
void set_ptr(pointer_and_flags* p, void* value) {
...
\arnote{TODO: highlight the added cast}
...
// Compiler warns: alignment (8) of 'struct AtLeast8ByteAligned' is less than the required capability alignment (16})
\end{clisting}
\begin{compilerwarning}
...
\end{clisting}
%
In the rare case that creating a potentially underaligned pointer is actually intended, th warning can be silence by adding a \ccode{annotate("underaligned\_capability")} attribute:
%
\begin{clisting}[frame=single]
...
Peter Sewell, Brett Gutstein, Alfredo Mazzinghi, Lee Smith, and Paul Gotch.
