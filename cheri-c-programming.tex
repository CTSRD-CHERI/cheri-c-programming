...
We also describe the most commonly encountered differences between these
...
CHERI Clang/LLVM and LLD implement the following new language and
code-generation/linkage models:
...
\item[Pure-capability code] is machine code that utilizes CHERI capabilities
  to implement all pointers, explicit (i.e., those explicitly declared by the
  programme) and implied (i.e., those implicitly used to implement the execution
  environment, such as for return addresses and pointers).
  Pure-capability code is not binary compatible with capability-unaware code
  utilizing integer pointers, not least due to the different size of the
  pointer data type.
\end{description}
...
\item It is able to travel at least \sfrac{1}{8} the size of the object, or $1$ KiB ($2^{floor(bounds\_bits/2)-3}$),
...
size_t allocate_next(struct Buffer *buf, size_t len) {
  ...
  buf->allocated += __builtin_cheri_round_representable_length(len);
