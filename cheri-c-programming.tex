...
% \subsection{Pointer equality}
%
% Pure-capability CHERI C and C++ implement pointer equality testing by
% comparing the addresses held in a pair of capabilities.
% For most purposes, this gives the behavior expected by programmers, as it
% tests whether the capabilities refer to the same memory location.
% However, in some rare circumstances, this can lead to confusing results.
% For example, if \texttt{realloc()} returns a pointer to the same location
% in memory, but with different bounds, the pointer value it returns will
...
\item[\texttt{void *cheri\_andperm(void *c, size\_t x)}] Perform a bitwise-AND of capability
  ...
  (See Section~\ref{sec:available_permissions}.)
%  This macro wraps the compiler built-in
%  \texttt{\_\_builtin\_cheri\_perms\_and()}.
...
%  This macro wraps the compiler built-in
%  \texttt{\_\_builtin\_cheri\_bounds\_set()}
...
%  This macro wraps the compiler built-in
%  \texttt{\_\_builtin\_cheri\_address\_set()}.
\end{description}

% \note{Are the references to the \texttt{\_\_builtin\_} forms useful?  Do we
% want to encourage their use or the \texttt{cheric.h} macros?}{nwf}
