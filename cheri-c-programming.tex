...
\begin{description}
\item[Pointer equality] Pointer \cvar{a} and pointer \cvar{b} are equal
...
\item[Pointer comparison] All pointers with valid tags compare as greater than
  ...
  If their tag values are equal, then pointer \cvar{a} compares greater than
  pointer \cvar{b} if \cvar{a}'s address is greater than
  \cvar{b}'s address.
\end{description}
...
implementing \ccode{==} (more lax) vs. in compiler optimization (more strict)
...
% For example, if \cfunc{realloc} returns a pointer to the same location
...
\note{I think we should mention the \ccode{X + offset < Y} vs.\@ \ccode{Y - X
> offset} issue here, even if we don't get into the full mess of pointer
comparisons, just to say that \ccode{X + offset} might result in an un-tagged
pointer.}{nwf}
