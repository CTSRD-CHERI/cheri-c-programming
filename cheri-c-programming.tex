...
\usepackage{listings}
\lstnewenvironment{clisting}[1][]{\lstset{language={C},breaklines=true,#1}}{}
\lstnewenvironment{compilerwarning}[1][]{\lstset{language={},breaklines=true,basicstyle=\scriptsize\ttfamily\bfseries,frame=single#1}}{}
...
%  as we continue to refine CHERI C/\cpp{} and its toolchain.}
...
\item[Permissions] The permissions mask controls how the capability can be
  used -- for example, by providing
  \psnote{[enabling/providing]}
  loading and storing of data and/or
  capabilities.
...
\psnote{before this, the text was talking about 64 and 128-bit caps; now it's silently shifted to only talking about 128-bit caps}
...
Other types that contain pointers, such as \cintptrt,
...
\item[\ptrdifft] This integer type describes the integer distance
  between the addresses of two pointers to within the same underlying allocation, and should not be used for
any other purpose.
...
\item[\cintptrt, \cuintptrt] These integer types should be
  ...
  \note{The observable, integer range of a \cuintptrt is the same as
...
It is important to use \ccode{alignof} instead of \ccode{sizeof} since GCC defines \maxalignt as a \ccode{struct} and not a \ccode{union}.} if pointers are to be stored in the allocation.

\item[\ccharstar, \ldots{}] These pointer types are suitable for
  dereference, but in general \psnote{that ``in general'' makes me wonder about the exceptions?}  should not be cast to or from arbitrary integer
  values.
...
%
The recommended approach to resolve such ambiguous cases is to change the type of one operand to to a non-provenance-carrying type such as \sizet.
...
We also provide a new attribute \ccode{cheri\_no\_provenance} that can be used to annotate variables or fields where the underlying type cannot be changed:
\begin{lstlisting}[language=C]
struct S {
...
\end{lstlisting}
\psnote{This doesn't really explain what \ccode{cheri\_no\_provenance} does?  And what it means when applied to other types?}
...
\item It is able to travel at least \sfrac{1}{8} the size of the object, or $1$ KiB,
...
\psnote{That's a strong claim -- better qualify w.r.t. that that has been ported?}
...
warning when using bitwise-AND on capability types such as \cuintptrt. \arnote{FIXME: This is not longer true now that we infer provenance, delete most of this}
...
\psnote{should the non-usability of high bits, mentioned at the end of \S3, also be mentioned here?}
...
\item[\ccode{uintptr\_t cheri\_clear\_low\_ptr\_bits(uintptr\_t ptr, vaddr\_t mask)}]
  This builtin clears the low bits of \cvar{ptr} in the same way as \ccode{ptr \& \textasciitilde{}mask}.
  It returns a new \cuintptrt value that can be used for memory accesses when cast to a pointer.
  \cvar{mask} should be a bitwise-AND mask less than \ccode{\_Alignof(ptr) - 1}.
\end{description}

\note{Is that \texttt{cheri.h} the same as the \texttt{cheri/cheri.h} mentioned earlier or a new file?}{nwf}
\arnote{Different file. I should update the compiler-provided one and use that in cheri/cheric.h}
...
\item[\ccode{vaddr\_t cheri\_getaddress(void *c)}] Return the address of the capability \cvar{c}.
%  This macro wraps the compiler built-in
...
\item[\ccode{size\_t cheri\_getperm(void *c)}] Return the permissions of capability
  \cvar{c}.
  (See \Cref{sec:available_permissions}.)
%  This macro wraps the compiler built-in
...
\item[\ccode{void *cheri\_setbounds(void *c, size\_t x)}] Narrow the bounds of capability
  ...
  wider than the range [\ccode{cheri\_getaddress(c)},
  \ccode{cheri\_getaddress(c) + x}) due to capability compression (see
...
\item[\ccode{void *cheri\_setaddress(void *c, vaddr\_t a)}] Return a new capability with the same permissions and bounds as \cvar{c} with the address set to \cvar{a}.
This builtin can be useful to re-derive a valid pointer from an address.

\ccode{cheri\_setaddress} is able to set an address \cvar{a} that is
...
\ccode{cheri\_setaddress} returns an untagged capability.
...
When the length of an object exceeds $4$ KiB, additional alignment requirements
...
The alignment required for allocations over $4$ KiB is $2^{E+3}$ bytes, where
...
Correctly computing the rounded size and minimum alignment for a given allocation is non-trivial and may require many instructions to compute.
Moreover, the exact limits for precise representability may vary across different implementations of CHERI.
Therefore, the CHERI ISA provides instructions that allow determining precisely representable allocations that
\psnote{???}
These instructions can be generated using the following compiler builtins:
\begin{description}
\item[\ccode{size\_t \_\_builtin\_cheri\_round\_representable\_length(size\_t len)}] returns the length that a capability would have after using \ccode{cheri\_setbounds} to set the length to \ccode{len} (assuming appropriate alignment of the base).

\item[\ccode{size\_t \_\_builtin\_cheri\_representable\_alignment\_mask(size\_t len)}] returns a bitmask that can be used to align an address downwards such that it is sufficiently aligned to create a precisely bounded capability.
\end{description}

The precisely representable base address can be computed using:
\begin{lstlisting}[language={C}]
	base = base & __builtin_cheri_representable_alignment_mask(len);
\end{lstlisting}
If you\psnote{awkward change of style to ``you''s} are allocating from a contiguous buffer, you will need to align the base upwards instead of down.
This can be done with the following code:
\begin{lstlisting}[language={C}]
size_t required_alignment(size_t len) {
  return ~__builtin_cheri_representable_alignment_mask(len) + 1;
}
struct Buffer {
  void* data;
  size_t allocated;
}
size_t allocate_next(struct Buffer* buf, size_t len) {
  void* result = buf->data + buf->allocated;
  result = __builtin_align_up(result, required_alignment(len));
  buf->allocated += __builtin_round_representable_length(len);
  return cheri_setbounds(result, len);
}
...
One use case \psnote{of what?} is high-performance applications that contain custom memory
...
\arnote{TODO: Sensible formatting}
...
\begin{clisting}[frame=single,numbers=left]
...
<source>:2:17: warning: cast from provenance-free integer type to pointer type will give pointer that can not be dereferenced [-Wcheri-capability-misuse]
  return strdup((const char*)ptr_or_int);
                ^
<source>:2:17: note: insert cast to intptr_t to silence this warning
1 warning generated.
\end{compilerwarning}
...
<source>:4:24: warning: cast from provenance-free integer type to pointer type will give pointer that can not be dereferenced [-Wcheri-capability-misuse]
  invoke_cb(&callback, (void*)callback_data); // warning
                       ^
<source>:4:24: note: insert cast to intptr_t to silence this warning
<source>:15:24: warning: cast to 'void * __capability' from smaller integer type 'int' [-Wint-to-void-pointer-cast]
  invoke_cb(&callback, (void*)callback_data); // warning
                       ^
...
\end{compilerwarning}
%
The simplest fix for this issue is to either increase alignment to be CHERI-compatibile, or use a ternary expression to include \ccode{alignof(\cvoidstar)}:
%
\begin{clisting}[frame=single]
