...
\lstnewenvironment{compilerwarning}[1][]{\lstset{language={},breaklines=true,basicstyle=\small\ttfamily\bfseries,#1}}{}
\subsection*{\texttt{cast from provenance-free integer type to pointer type will give pointer that can not be dereferenced}}
...
int example_bad(long ptr_or_int) {
  return strdup((const char*)ptr_or_int); // warning triggers here
}
int example_good(intptr_t ptr_or_int) {
  return strdup((const char*)ptr_or_int); // no warning
}
\end{clisting}
...
\begin{clisting}[frame=single]
int false_positive_example(int callback_data) {
  invoke_cb(&callback, (void*)callback_data); // warning triggers here
  invoke_cb(&callback, (void*)(uintptr_t)callback_data); // no warning
}
\end{clisting}

\subsection*{\texttt{alignment of '<type>' is less than the required capability alignment}}
