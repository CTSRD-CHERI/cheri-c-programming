...
| Be sufficiently aligned to allow capability loads and stores at relative offset 0 from the returned pointer, if the allocation is at least the size of a capability | As with capability load and store permissions, being unable to use pointers stored at pointer alignment within an alllocation will break almost all extant software. |
...
| Requirement | Rationale |
|-------------|-----------|
| Implement pointer revocation by clearing the tag of capabilities pointing to freed memory | Clearing the tag ensures non-dereferenceability when a pointer is revoked, although this can also be achieved by other means such as changing page-table permissions on the page a pointer points to, on supporting architectures. |
