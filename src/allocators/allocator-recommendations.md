...
| Be sufficiently aligned to allow capability loads and stores at relative offset 0 from the returned pointer, if the allocation is at least the size of a capability |
 | As with capability load and store permissions, being unable to use pointers stored at pointer alignment within an alllocation will break almost all extant software. |
